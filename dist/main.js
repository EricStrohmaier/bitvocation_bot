...
import { createUserEntry, deleteJobAlerts, fetchAndPostLatestEntries, formatVariables, getKeyword, getLatestJobs, getUserConfigs, handlePrivacy, hasJobAlert, readUserEntry, sendParseMessage, updateJobAlerts, } from './functions';
...
bot.on('message', async (msg) => {
    ...
    // }
    if (setJobAlert && ((_a = msg.text) === null || _a === void 0 ? void 0 : _a.startsWith('/')) && !msg.text.startsWith('/jobalert')) {
        setJobAlert = false;
        await bot.sendMessage(chatId, 'Something went wrong. Please try again to set up a /jobalert.');
    }
    if (setJobAlert) {
...
bot.onText(/^\/(\w+)(@\w+)?(?:\s.\*)?/, async (msg, match) => {
    ...
    if (match.input.split(' ').length != 1) {
        ...
        const userLanguage = ((_a = userConfigs[chatId]) === null || _a === void 0 ? void 0 : _a.language) || PARAMETERS.LANGUAGE;
        ...
        if (!(command.startsWith('/start') ||
            ...
            await bot.sendMessage(msg.chat.id, formatVariables(TRANSLATIONS[userLanguage].errors['generic-error'], {
                command,
    ...
    const chatId = msg.chat.id.toString();
    const userConfigs = getUserConfigs();
    const userLanguage = ((_b = userConfigs[chatId]) === null || _b === void 0 ? void 0 : _b.language) || PARAMETERS.LANGUAGE;
    setJobAlert = false;
    switch (command) {
        case '/start':
            (async () => {
